...
class BackwardPage extends StatefulWidget {
  const BackwardPage({Key? key}) : super(key: key);

  @override
  _BackwardPageState createState() => _BackwardPageState();
}

class _BackwardPageState extends State<BackwardPage> {
  // stroage & upload
    ...
    _timer = new Timer.periodic(Duration(seconds: 1), (timer) {
      if (timer.tick > endSeconds) {
        ...
        setState(() {
          ...
          setBtnText();
          storage.Upload(_account, '13', score); //Upload
        });
      } else {
        setState(() {
      ...
      body: Column(
        ...
        children: [
          if (_gamestatus == GameState.IDLE || _gamestatus == GameState.END)
            Center(
              child: Text(
                _gamestatus == GameState.IDLE ? "逆向回憶" : "遊戲結束",
                style: TextStyle(
                  fontSize: 48.0,
                          ...
                          if (_gamestatus != GameState.IDLE &&
                            ...
                            if (_game.matchCheck.length > 0) {
                              ...
                              for (var i = 0; i < _game.matchCheck.length; i++) {
                                if (_game.matchCheck[i] != _game.reverseOrder![i]) {
                                  isCorrect = false;
                                  break;
                                }
                              }
